...
import { Menu, Github, ExternalLink, LogOut, Loader2, Trash2 } from "lucide-react";
import { DeleteAccountDialog } from "@/components/auth/delete-account-dialog";
import { useRouteLocale } from "@/lib/i18n/use-route-locale";
import { useRootLocale } from "@/lib/i18n/use-root-locale";
...
function AuthControls({ compact = false }: { compact?: boolean }) {
  ...
  if (!session) {
    return (
      <SignInDialog
        trigger={
          <Button size={compact ? "sm" : "default"} variant="default">
            登录
          </Button>
        }
      />
  ...
  return (
    <div className={cn("flex items-center", compact ? "gap-2" : "gap-3")}>
      ...
      <DeleteAccountDialog
        trigger={
          <Button size={compact ? "sm" : "default"} variant="ghost" title="删除账户" className="text-destructive hover:text-destructive">
            <Trash2 className="h-4 w-4" />
            <span className={compact ? "sr-only" : ""}>删除账户</span>
          </Button>
        }
      />
      <Button
      ...
      >
        ...
        <span className={compact ? "sr-only" : ""}>登出</span>
      </Button>
    </div>
  );
}
...
export function NavigationBar() {
  ...
  const currentLocale = useRootLocale();
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="hidden lg:flex items-center space-x-1">
            ...
            <div className="ml-2 pl-2 border-l border-border/30 flex items-center gap-1">
              <LanguageSwitcher currentLocale={currentLocale} align="end" />
